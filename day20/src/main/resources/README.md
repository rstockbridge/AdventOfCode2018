...
The rest of the regex matches various sequences of the characters `N` (north), `S` (south), `E` (east), and `W` (west). In the example above, `^WNE$` matches only one route, `WNE`, which means you can advance west, then north, then east from your current position. Sequences of letters like this always match that exact route in the same order.
...
Now, only `SSE(EE|N)` remains. Because it is in the same parenthesized group as `NEEE`, it starts from the same room `NEEE` started in. It states that starting from that point, there exist doors which will allow you to advance south twice, then east; this ends up at another branch. After that, you can either advance east twice or north once. This information fills in the rest of the doors:
...
Sometimes, a list of options can have an empty option, like `(NEWS|WNSE|)`. This means that routes at this point could effectively skip the options in parentheses and advance on immediately. For example, consider this regex and the corresponding map:
